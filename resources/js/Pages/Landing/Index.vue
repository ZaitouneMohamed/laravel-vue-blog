                    ...
                    <div class="mt-6" v-for="item in posts">
                        <div class="max-w-4xl px-10 py-6 mx-auto bg-white rounded-lg shadow-md">
                            <div class="flex items-center justify-between"><span class="font-light text-gray-600">{{
                                item.created_at }}</span><a href="#"
                                    class="px-2 py-1 font-bold text-gray-100 bg-gray-600 rounded hover:bg-gray-500">{{
                                        item.categorie.name }}</a>
                            </div>
                            <div class="mt-2"><Link :href="route('post.show', item)" class="text-2xl font-bold text-gray-700 hover:underline">{{
                                item.title }}</Link>
                                <p class="mt-2 text-gray-600">{{ item.title.substring(0, 500) + ".." }}</p>
                            </div>
                            <div class="flex items-center justify-between mt-4">
                                <Link :href="route('post.show', item)" class="text-blue-500 hover:underline">Read more</Link>
                                <div>
                        ...
                        <div class="flex flex-col max-w-sm px-6 py-4 mx-auto bg-white rounded-lg shadow-md">
                            <ul class="-mx-4">
                                <li class="flex items-center mt-6" v-for="item in $page.props.topUsers"><img
                                    ...
                                    }}</a><span class="text-sm font-light text-gray-700">Created {{ item.posts_count
}} Posts</span></p>
                                </li>
                            </ul>
                        </div>
